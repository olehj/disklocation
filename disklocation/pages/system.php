<?php
	...
	if(isset($_POST["save_settings"])) {
		...
		if($_POST["device_bg_color"] !== 1 && !preg_match("/#([a-f0-9]{3}){1,2}\b/i", $_POST["bgcolor_flash"])) { $disklocation_error[] = "Background color for \"Boot/Flash\" invalid."; } else { $_POST["bgcolor_flash"] = str_replace("#", "", strtoupper($_POST["bgcolor_flash"])); }
	...
	if(isset($_POST["save_groupsettings"])) {
		...
		foreach($new_array as $id => $setting) {
			...
			if($new_array[$id]["tray_align_txt"] && !preg_match("/\b(left|right|center|vertical|hide)\b/", $new_array[$id]["tray_align_txt"])) { $disklocation_error[] = "" . $new_array[$id]["group_name"] . ": Wrong alignment value."; }
